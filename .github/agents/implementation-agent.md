...
5. **作業ブランチの作成**: 承認後、実装開始前に必ず新しいブランチを作成する

### 作業ブランチの作成（必須）

**実装を開始する前に、必ず新しい作業ブランチを作成すること:**

```bash
git checkout -b feature/[機能名]  # または fix/[修正内容]
```
...
- ブランチ名のプレフィックス:
  ...
  - `docs/` - ドキュメント更新のみ
  - `chore/` - 設定変更やメンテナンス
- ブランチ名は具体的でわかりやすく（例: `feature/transaction-list`, `fix/auth-redirect`）
- 作業開始前にブランチを作成することで、mainブランチを保護し、変更を整理する
